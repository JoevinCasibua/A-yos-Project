...
const generateTickets = (count) => {
  ...
  return Array.from({ length: count }, (_, i) => ({
    id: `TCK-${8000 + i}`,
    customer: `User ${Math.floor(Math.random() * 200) + 1}`,
    subject: `Issue with ${categories[Math.floor(Math.random() * categories.length)]}`,
    category: categories[Math.floor(Math.random() * categories.length)],
    priority: priorities[Math.floor(Math.random() * priorities.length)],
    status: statuses[Math.floor(Math.random() * statuses.length)],
    date: new Date(Date.now() - Math.random() * 5000000000).toISOString().split('T')[0],
    assignedTo: Math.random() > 0.3 ? `Agent ${Math.floor(Math.random() * 5) + 1}` : 'Unassigned',
  }));
};
...
const Support = () => {
  ...
  const stats = [
    ...
    { label: 'Resolved Today', value: 12, icon: <CheckCircle className="text-green-500" />, bg: 'bg-green-50' },
  ];
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paginatedTickets.length > 0 ? paginatedTickets.map((ticket) => (
              <tr key={ticket.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => openTicket(ticket)}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className={`text-sm ${ticket.assignedTo === 'Unassigned' ? 'text-gray-400 italic' : 'text-gray-900'}`}>
                    {ticket.assignedTo}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="p-6 border-b border-gray-200 bg-gray-50 shrink-0">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium flex items-center mt-1 text-right">{selectedTicket.assignedTo}</p>
                </div>
              </div>
            </div>
              ...
              <div className="flex">
                ...
                <div>
                  <div className="bg-gray-100 rounded-lg p-3 text-sm text-gray-800 rounded-tl-none">
                    <p>Hi, I am having an issue with my recent booking {selectedTicket.subject.toLowerCase()}. The worker charged me but didn't finish the job.</p>
                  </div>
                  <p className="text-xs text-gray-400 mt-1">{selectedTicket.date} 09:14 AM</p>
                </div>
              </div>
              
              <div className="flex flex-row-reverse">
                <div className="flex-shrink-0 ml-3">
                  <div className="h-8 w-8 rounded-full bg-gray-800 flex items-center justify-center text-white font-bold text-xs">
                    A
                  </div>
                </div>
                <div>
                  <div className="bg-blue-600 rounded-lg p-3 text-sm text-white rounded-tr-none">
                    <p>Hello! I am sorry to hear about this. Let me look into this booking immediately. Could you please provide the booking ID?</p>
                  </div>
                  <p className="text-xs text-gray-400 mt-1 text-right">{selectedTicket.date} 09:45 AM</p>
                </div>
              </div>
