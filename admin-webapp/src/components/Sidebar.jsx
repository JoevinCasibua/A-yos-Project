  ...
  if (group.isLink) {
    ...
    return (
      <div className="mb-4 px-3">
        ...
        >
          <GroupIcon className={`shrink-0 ${effectiveCollapsed ? 'h-5 w-5' : 'h-5 w-5 mr-3'} ${
  ...
  return (
    <div className="mb-4 px-3">
      ...
      >
        ...
        {!effectiveCollapsed && (
          <span className={`transition-transform duration-200 ${isExpanded ? 'rotate-180 text-gray-800' : 'text-gray-400'}`}>
            <ChevronDown size={16} />
          </span>
        )}
      </button>
        ...
        <div className="pl-11 pr-2 space-y-1.5 pt-1">
          {(group.items || []).map((item) => {
            ...
            return (
              ...
              >
                <ItemIcon className={`h-4 w-4 mr-3 shrink-0 transition-colors ${isItemActive ? 'text-primary' : 'text-gray-500'}`} />
                {item.name}
              </NavLink>
            );
          })}
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between h-16 px-4 shrink-0">
          <div className={`flex items-center ${effectiveCollapsed && !isMobileOpen ? 'mx-auto' : ''}`}>
          ...
          {isMobileOpen && (
            <button onClick={() => setIsMobileOpen(false)} className="md:hidden p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg transition-colors">
              <X className="h-5 w-5" />
            </button>
          )}
        </div>
        ...
        <div className="p-4 border-t border-white/10 space-y-2 shrink-0">
          ...
          >
            {({ isActive }) => (
              <>
                <UserCircle className={`shrink-0 ${effectiveCollapsed && !isMobileOpen ? 'h-5 w-5' : 'h-5 w-5 mr-3'} ${
                  isActive ? 'text-primary' : 'text-gray-400 group-hover:text-gray-200'
                }`} />
          ...
          >
            <LogOut className={`shrink-0 ${effectiveCollapsed && !isMobileOpen ? 'h-5 w-5' : 'h-5 w-5 mr-3'}`} />
