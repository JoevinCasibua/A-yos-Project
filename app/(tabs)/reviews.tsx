...
import { reviews, providers } from '@/constants/mockData';
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <AppText variant="h2" weight="bold">Reviews</AppText>
      </View>
...
function PressableFilterChip({ label, selected, onPress }: { label: string; selected: boolean; onPress: () => void }) {
  return <Chip label={label} selected={selected} size="sm" onPress={onPress} />;
}

function PressableLikeBtn({ liked, onPress }: { liked: boolean; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={styles.likeBtn} hitSlop={8}>
      <ThumbsUp size={14} color={liked ? Colors.cta : Colors.textTertiary} fill={liked ? Colors.cta : 'transparent'} strokeWidth={2} />
      <AppText variant="caption" weight="medium" color={liked ? Colors.cta : Colors.textTertiary}>
        Helpful {liked ? '1' : ''}
      </AppText>
    </Pressable>
  );
