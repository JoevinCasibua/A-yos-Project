...
begin
  ...
  if decision='approved' and (not public.has_approved_id(before_row.user_id) or not exists(select 1 from public.worker_services where worker_id=application_uuid and price_centavos > 0)) then raise exception using errcode='P0001', message='WORKER_INELIGIBLE'; end if;
