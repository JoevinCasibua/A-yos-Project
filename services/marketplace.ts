...
export async function analyzeRequest(description: string, photoUri?: string): Promise<ServiceResult<AnalysisResult>> {
  try {
    ...
    if (photoUri) {
      const response = await fetch(photoUri); const bytes = new Uint8Array(await response.arrayBuffer());
...
async function uploadRequestPhoto(userId: string, uri: string) {
  const response = await fetch(uri); if (!response.ok) throw new Error('VALIDATION_FAILED');
...
export async function fetchRecommendations(requestId: string): Promise<ServiceResult<WorkerRecommendation[]>> {
  try {
    const { data, error } = await supabase.from('recommendations').select('worker_id,rank,explanation,worker_applications!inner(rating,review_count,experience_years,profiles!inner(first_name,last_name,avatar_path),worker_services!inner(title,price_centavos))').eq('request_id', requestId).order('rank');
    if (error) throw error;
...
export async function submitBookingReview(bookingId:string,rating:number,comment:string):Promise<ServiceResult<{id:string}>>{
  try{const {data,error}=await supabase.rpc('submit_review',{booking_uuid:bookingId,stars:rating,review_comment:comment.trim()||null});if(error)throw error;return{data:{id:data.id},error:null};}
  catch(error){return{data:null,error:toServiceError(error)};}
}

export async function confirmBookingComplete(bookingId:string):Promise<ServiceResult<true>>{
  try{const {error}=await supabase.rpc('transition_booking',{booking_uuid:bookingId,next_status:'completed',note:'Customer confirmed service completion.',note_lang:'en'});if(error)throw error;return{data:true,error:null};}
  catch(error){return{data:null,error:toServiceError(error)};}
}
